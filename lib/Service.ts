...
import { visit as v, builders as t } from "ast-types";
import Base from "./Base";
import { formatPaths } from "./utils";
import { formatSchema, formatRefName } from "./utils/format";
import { ResCode, PathsType, Paramster } from "./types/base";
...
export default class BuildService extends Base {
  ...
  addImportNode(path: VisitPath) {
    // request import
    const imporNode = path.node.body.find(
    ...
    if (!imporNode) {
      ...
      path
        .get("body")
        .unshift(
          super.getImportDeclaration(["ResType"], "../lib/types/base.d"),
        );
    }
    // 类型引入
    path
      .get("body")
      .unshift(
        super.getImportDeclaration(
          Array.from(new Set(this.importType)),
          this.importTypePath,
        ),
      );
  }
  addExportNode(paths: PathsType, path: VisitPath) {
    formatPaths(paths).forEach((i, index) => {
      ...
      if (!sameExportNode) {
        ...
        path
          .get("body")
          .push(t.exportNamedDeclaration(this.getFunctionDeclaration(i)));
        // 通过方法index映射注释
